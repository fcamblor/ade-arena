...
const data: OrchestratorVersion = {
  ...
  features: [
    ...
    },
    {
      ...
      sourceExtract: 'Pass `--worktree` or `-w` to create an isolated worktree and start Claude in it. By default, the worktree is created under `.claude/worktrees/<value>/` at your repository root, on a new branch named `worktree-<value>`.',
    },
    {
      ...
      sourceExtract: 'Claude Code on the web runs tasks on Anthropic-managed cloud infrastructure at claude.ai/code. Sessions persist even if you close your browser, and you can monitor them from the Claude mobile app.',
    },
    {
      ...
      sourceExtract: 'Claude Code is an agentic coding tool that reads your codebase, edits files, runs commands, and integrates with your development tools. Available in your terminal, IDE, desktop app, and browser.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/model-config',
      sourceExtract: 'For the `model` setting in Claude Code, you can configure either: A model alias — A model name — Anthropic API: A full model name — Bedrock: an inference profile ARN — Foundry: a deployment name — Vertex: a version name.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/overview',
      sourceExtract: 'Claude Code works directly with git. It stages changes, writes commit messages, creates branches, and opens pull requests.',
    },
    {
    ...
    },
    {
      ...
      sourceExtract: 'After Claude makes changes to your code, the diff view lets you review modifications file by file before creating a pull request.',
    },
    {
      featureId: 'oss',
      support: 'no',
    ...
    },
    {
      ...
      sourceExtract: 'Hooks are user-defined shell commands, HTTP endpoints, or LLM prompts that execute automatically at specific points in Claude Code\'s lifecycle.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/agent-view',
      sourceExtract: 'Before editing files, Claude moves the session into an isolated git worktree under `.claude/worktrees/`, so parallel sessions can read the same checkout but each writes to its own.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/quickstart',
      sourceExtract: 'Open your terminal in any project directory and start Claude Code: `cd /path/to/your/project` `claude`.',
    },
    {
    ...
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/desktop',
      sourceExtract: 'Claude automatically detects your dev server setup and stores the configuration in `.claude/launch.json` at the root of the folder you selected when starting the session. You can define multiple configurations to run different servers from the same project, such as a frontend and an API.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/desktop',
      sourceExtract: 'When Claude picks a different port, it passes the assigned port to your server via the `PORT` environment variable.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/overview',
      sourceExtract: 'Claude Code is an agentic coding tool that reads your codebase, edits files, runs commands, and integrates with your development tools.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/desktop',
      sourceExtract: 'Click this indicator to open the diff viewer, which displays a file list on the left and the changes for each file on the right.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/desktop',
      sourceExtract: 'To comment on specific lines, click any line in the diff to open a comment box. Type your feedback and press Enter to add the comment. Claude reads your comments and makes the requested changes, which appear as a new diff you can review.',
    },
    {
    ...
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/desktop',
      sourceExtract: "The integrated terminal lets you run commands alongside your session without switching to another app. The terminal opens in your session's working directory and shares the same environment as Claude, so commands like `npm test` or `git status` see the same files Claude is editing.",
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/desktop',
      sourceExtract: 'Right-click any file path in the chat, diff viewer, or file pane to open a context menu: […] Open in: open the file in an installed editor such as VS Code, Cursor, or Zed.',
    },
    {
    ...
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/desktop',
      sourceExtract: 'Click a file path in the chat or diff viewer to open it in the file pane. Make spot edits and click Save to write them back. If the file changed on disk since you opened it, the pane warns you and lets you override or discard.',
    },
    {
      ...
      sourceExtract: 'Custom commands have been merged into skills. A file at `.claude/commands/deploy.md` and a skill at `.claude/skills/deploy/SKILL.md` both create `/deploy` and work the same way.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/overview',
      sourceExtract: "Sessions aren't tied to a single surface. Move work between environments as your context changes: Kick off a long-running task on the web or iOS app, then pull it into your terminal with `claude --teleport`. Hand off a terminal session to the Desktop app with `/desktop` for visual diff review.",
    },
    {
    ...
    },
    {
      ...
      sourceExtract: 'User settings: `~/.claude/settings.json` — apply to all projects. Project settings: `.claude/settings.json` — checked into source control and shared with team. Local settings: `.claude/settings.local.json` — personal preferences, not checked in, automatically gitignored.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/desktop',
      sourceExtract: 'After you open a pull request, a CI status bar appears in the session. […] To archive the session automatically once the PR merges or closes, turn on auto-archive in Settings → Claude Code.',
    },
    {
      ...
      sourceExtract: 'Remote Control connects claude.ai/code or the Claude app for iOS and Android to a Claude Code session running on your machine. Start a task at your desk, then pick it up from your phone on the couch or a browser on another computer.',
    },
    {
      ...
      sourceExtract: "Context indicator: the prompt box shows how much of Claude's context window you're using. Claude automatically compacts when needed, or you can run `/compact` manually.",
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/model-config',
      sourceExtract: 'During session — Use `/model <alias|name>` to switch immediately, or run `/model` with no argument to open the picker. Your `/model` selection is saved to user settings and persists across restarts.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/model-config',
      sourceExtract: 'Effort levels control adaptive reasoning, which lets the model decide whether and how much to think on each step based on task complexity. Opus 4.7: `low`, `medium`, `high`, `xhigh`, `max`.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/model-config',
      sourceExtract: 'Available models: opus, sonnet, haiku, opusplan, sonnet[1m], opus[1m]. On the Anthropic API and Claude Platform on AWS, `opus` resolves to Opus 4.7 and `sonnet` resolves to Sonnet 4.6.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/desktop',
      sourceExtract: 'Claude can start a dev server and open an embedded browser to verify its changes. This works for frontend web apps as well as backend servers: Claude can test API endpoints, view server logs, and iterate on issues it finds.',
    },
    {
      ...
      sourceExtract: 'Plugins let you extend Claude Code with custom functionality that can be shared across projects and teams. This guide covers creating your own plugins with skills, agents, hooks, and MCP servers.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/quickstart',
      sourceExtract: '`claude -p "query"` — Run one-off query, then exit — `claude -p "explain this function"`.',
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/agent-view',
      sourceExtract: "The list shows every background session you've started, across all your projects. A session working in one repository and another in a different worktree both appear here, regardless of which directory you opened agent view from.",
    },
    {
      ...
      screenshots: [],
      sourceUrl: 'https://code.claude.com/docs/en/worktrees',
      sourceExtract: 'A worktree is a fresh checkout, so untracked files like `.env` or `.env.local` from your main repository are not present. To copy them automatically when Claude creates a worktree, add a `.worktreeinclude` file to your project root.',
    },
    {
